...
A self-paced course that teaches AI fluency to biologists by running a single-cell RNA-seq pipeline (FASTQ to annotated UMAP) with an AI assistant in the loop. Plan on roughly one week per unit at 3–5 hours; allow 4–8 weeks total. No instructor, no grades, fully solo.
...
2. AI literacy for bio — how LLMs work, prompting, tool use and agents, ethics and limits (Week 2).
3. Hands-on bioinformatics — code assistance, data analysis, literature review, protocol design — grounded in the scRNA-seq project (Weeks 3 and 4).
...
| [2](modules/module-02-alignment-count-matrix/README.md) | Alignment and count matrix (Cell Ranger / STARsolo) | HPC (or skip — pre-built matrix in Module 3) | Week 3 reading |
| [3](modules/module-03-preprocessing-scanpy/README.md) | Preprocessing and QC in Scanpy | Google Colab (free) | Week 3 hands-on |
| [4](modules/module-04-clustering-umap/README.md) | Dimensionality reduction, clustering, UMAP | Google Colab (free) | Week 3 hands-on |
| [5](modules/module-05-annotation-interpretation/README.md) | Cell-type annotation and differential expression | Google Colab (free) | Week 4 hands-on |

Dataset: 10x PBMC 3k throughout — 2,700 PBMCs from a healthy donor, roughly 8 well-characterised cell types, small enough to run in Colab on a free tier.

Suggested pacing (roughly one week per unit at 3–5 hours; allow 4–8 weeks total):
...
│   └── protocol-design.qmd
├── modules/                    # scRNA-seq pipeline (FASTQ to annotated UMAP)
