...
- [Prompting](../literacy/prompting.qmd). Focus on the weak-to-strong progression and the role-task-context-format anatomy. The mermaid diagram is worth pausing on.
...
2. *Fix*. Apply one change from [Prompting](../literacy/prompting.qmd): role plus task plus context plus format, few-shot, constrained output, and so on.
...
A common error is putting scientific judgement tasks (threshold choices, statistical test selection, interpretation) in the agent column. Re-read [Tool use & agents](../literacy/tool-use-and-agents.qmd) on when not to agent.
:::
...
1. Prompt comparison. Take a recurring task from your work. Submit (a) a weak prompt, (b) a strong prompt using patterns from [Prompting](../literacy/prompting.qmd), (c) outputs from each, and (d) a three- to five-sentence critique explaining what changed and why.
